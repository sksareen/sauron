...
One binary. One database. 11 MCP tools. Zero cloud dependencies.

<p align="center">
  <img src="assets/screenshot.png" alt="Sauron web UI — current thread, activity, interrupts" width="800">
</p>
...
### 3. Infers What You're Working On (HINT)

The daemon continuously infers your current intent as a **Human Intention Vector (HINT)** — a live, weighted label like `"building the Sauron inference engine"` generated by an LLM from recent activity evidence.

Each HINT has:
- **label** — LLM-inferred in plain English (e.g. `"reviewing auth PR"`)
- **weight** — decays over time (30-min half-life), spikes on new evidence
- **evidence** — linked signals: activity entries, clipboard items, sessions

Multiple HINTs can be active simultaneously (graph model, not a stack). The highest-weight labelled HINT surfaces as the current thread in the UI and in `sauron_context`.

```bash
sauron hints              # see active HINTs with labels + weights
sauron hints --all        # full history
```

The `sauron_context` tool now leads with `open_thread` and `next_action` — the semantic answer to "what was I doing?" — before showing mechanics like dominant app or focus score.

### 4. Screenshot Capture + Annotation
...
Once installed, Claude Code gets 11 tools it can call on demand:

### Watcher

| Tool | Returns |
|------|---------|
| `sauron_context` | Current open thread (HINT label), next action, recent decisions, dominant app, local servers |
...
### HINT — Human Intention Vectors

| Tool | Does |
|------|------|
| `sauron_hints` | Active HINTs with LLM-inferred labels, weight, confidence, and evidence — what the user is working on right now |
...
# ── HINT — Human Intention Vectors ────────────
sauron hints                      # active HINTs with labels + weights
sauron hints --all                # full history including paused/abandoned
sauron hints --limit 10           # max hints to return
...
sauron experience log \           # record a completed experience
  --task-intent "..." \
  --approach "..." \
